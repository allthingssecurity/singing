...
class ACEStepPipeline:
    def _emit_progress(self, step:int, total:int):
        ...
        if cb:
            try:
                cb({'step': int(step), 'total': int(total)})
            except Exception:
                pass
    ...
    def flowedit_diffusion_process(
        ...
        for i, t in tqdm(enumerate(timesteps), total=T_steps):
            self._emit_progress(i+1, T_steps)
    ...
    def text2music_diffusion_process(
        ...
        for i, t in tqdm(enumerate(timesteps), total=num_inference_steps):
            self._emit_progress(i+1, num_inference_steps)
    ...
    def save_wav_file(
        ...
        # Robust save across platforms/backends
        import subprocess, os
        if format.lower() == "mp3":
            # Save temp WAV then transcode to MP3 via ffmpeg
            tmp_wav = f"{base_path}/output_{time.strftime('%Y%m%d%H%M%S')}_{idx}.wav"
            torchaudio.save(tmp_wav, target_wav, sample_rate=sample_rate, format="wav")
            cmd = ["ffmpeg", "-y", "-i", tmp_wav, "-codec:a", "libmp3lame", "-b:a", "320k", output_path_flac]
            try:
                subprocess.run(cmd, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                os.remove(tmp_wav)
            except Exception as e:
                logger.warning(f"ffmpeg mp3 encode failed ({e}), falling back to WAV")
                output_path_flac = tmp_wav
        else:
